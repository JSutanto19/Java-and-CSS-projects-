...
public interface BALSTADT<K extends Comparable<K>, V> {
  ...
  /**
   * This method searches tree for node with key that is passed in to method and return the value
   * 
   * @return value of key of node inside the tree
   * @throws IllegalNullKeyException when key passed in is null
   * @throws KeyNotFoundException when node with key is not in the tree
   */
  V get(K key) throws IllegalNullKeyException, KeyNotFoundException;

  /**
   * Checks if there is a node with same key in tree
   * @throws IllegalNullKeyException If key is null
   * @return false if key is not null and is not present, returns true if the key is in the data
   *         structure
   */
