...
public class Main extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		loginBox.setBackground(new Background(new BackgroundFill(Color.DARKGREEN, CornerRadii.EMPTY, Insets.EMPTY)));
		...
		MenuItem fileSaveButton = new MenuItem("Save (ctrl + S)"); // TODO
					...
					try {
						...
						if (newCentralName != null) {
							...
							if (network.getAllUsers().size() > 0) {
								String name = network.getAllUsers().get(0).getName();
						...
						Alert success = new Alert(Alert.AlertType.INFORMATION,
								myFile.getName() + " has been successfully uploaded!");
						success.showAndWait();
					} catch (FileNotFoundException e) {
					} catch (InvalidSyntaxException e) {
						Alert error = new Alert(Alert.AlertType.ERROR, "Invalid format at line " + e.getLineNum()
			...
			public void handle(ActionEvent arg0) {
				...
				if (file != null) {
					try {
						...
						Alert success = new Alert(Alert.AlertType.INFORMATION,
								"Network has been successfully saved to " + file.getName());
						success.showAndWait();
					} catch (IOException e) {
					}
				}
		...
		EventHandler<ActionEvent> saveFileEventHandler = event -> {
			Alert saveConfirmation =
					...
					new Alert(Alert.AlertType.CONFIRMATION, "Do you want to save your current network?");
			...
			saveConfirmation.getButtonTypes().setAll(cancelButton, dontSaveButton, saveButton);
			// dont save when cancel is pressed
			Button cancelBut = (Button) saveConfirmation.getDialogPane().lookupButton(cancelButton);
			cancelBut.setDefaultButton(false);
			// save when save button is clicked
			Button saveBut = (Button) saveConfirmation.getDialogPane().lookupButton(saveButton);
			...
			if (saveResponse.get() == saveButton) {
				...
				Alert info = new Alert(Alert.AlertType.INFORMATION, LOG_FILE_NAME + " has been successfully saved!");
				info.showAndWait();
		...
		searchButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				try {
					...
					for (Person friend : allFriends) {
						if (friend.getName().toLowerCase().contains(search.getCharacters().toString().toLowerCase())) {
							list.add(friendButton(friend.getName()));
						}
					}
					// update friends list with friends that user wants to searched
					friendsList.getItems().setAll(list);
					underSearch.setText("Search under \"" + search.getCharacters() + "\"");
		...
		VBox friendListBox = new VBox(new Label("My Connections:"), friendListBoxSub, searchPack);

		// CLOSE OR EXIT
		EventHandler<WindowEvent> confirmCloseEventHandler = event -> {
			Alert closeConfirmation =
					// show exit window
					new Alert(Alert.AlertType.CONFIRMATION, "Do you want to exit Social Network?");
			...
			if (!ButtonType.OK.equals(closeResponse.get())) {
				...
				if (primaryStage.getScene().equals(mainScene)) {
					...
					if (saveResponse.get() == saveButton) {
						...
						info.setHeaderText("Thank you for using our Social Network!");
						info.showAndWait();
					} else if (saveResponse.get() == dontSaveButton) {
        ...
        MenuItem helpMenuButton = new MenuItem("Help (ctrl + H)");  // creates "Help" menu item
        ...
        helpMenuButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                try {
                    ...
                    while (scnr.hasNextLine()) {  // scans as concatenates contents of "Help" file
                        str += scnr.nextLine() + "\n";
                    }
                } catch (FileNotFoundException e) {
                }
        ...
        helpMenu.getItems().add(helpMenuButton);  // adds the help button to the Menu
        menu.getMenus().add(helpMenu);  // adds Help menu item to the Menu

        // create label called "Number of Connected Groups:"
        Label numGroups = new Label("Number of Connected Groups: " + network.numConnectedComponents());
        ...
        // "Number of connected components" to a VBox to be set to the right of the scene
        VBox rightBox = new VBox(friendListBox, numGroupsBox, friendScene.getRoot());
        ...
        mutualA.setValue("--select person A--");  // sets what to display on ComboBox
        ...
        mutualB.setValue("--select person B--");  // sets what to display on ComboBox
        mutualB.setVisibleRowCount(5);
        
        // creates a VBox containing labels of Person A and B (to name the ComboBoxes in the GUI)
        ...
        mutualList.setPrefSize(300, 100);  // sets the dimensions of the JavaFX ListView
        Button searchMutualButton = new Button("Search");  // creates button called "Search"
        
        // sets action for the button above to find the mutual friends between userA and userB
        searchMutualButton.setOnAction(event -> {
            ...
            if (mutualA.getValue().equals("--select person A--")
                ...
                try {
                    ...
                    List<String> strName = network.mutualConnections(mutualA.getValue(), mutualB.getValue());
        ...
        HBox mutualSub = new HBox(mutualList);  // create a HBox for list of mutual friends
        // creates a VBox to hold all info for mutual friends (ComboBox, Labels, ListViews)
        ...
        start.setValue("--select starting person--");  // sets what to display on ComboBox
        ...
        target.setValue("--select target person--");  // sets what to display on ComboBox
        target.setVisibleRowCount(5);

        // creates a VBox containing labels of Start and Target to identify the ComboBoxes
        ...
        // sets action for the button above to find the shortest connection between 2 users
        shortestButton.setOnAction(event -> {
            ...
            if (start.getValue().equals("--select starting person--")
                ...
                try {
                	// find shortest connection between 2 users using djikstraSort() and adding it
                	// to the ViewLsit called "shortestList"
                    shortestList.getItems().setAll(network.djikstraSort(start.getValue(), target.getValue()));
                } catch (VertexNotFoundException e) {  // exception handling
                }
            }
        });
        
        shortestList.setPrefSize(300, 100);  // setting dimensions for the ListView
        ...
        for (Person friend : network.getAllUsers()) {  // loop to get all users
            targetCentral.getItems().add(friend.getName());  // add all users to ComboBox
        }
        targetCentral.setValue("--select new central user--");  // labels the ComboBox
        targetCentral.setVisibleRowCount(5);
        
        //HERE
        // for "Set Central User" button (actions)
        Button setCentralButton = new Button("Set Central User"); // create button
        // set action for the button
        setCentralButton.setOnAction(event -> {
        	// make sure a user is selected
            if (targetCentral.getValue().equals("--select new central user--")) {
            	// show error when exception caught 
                Alert error = new Alert(Alert.AlertType.ERROR,
                ...
                centralUser.setText(targetCentral.getValue());  // updates Central User label in GUI
                try {
        ...
        HBox setCentralBox = new HBox(logoutButton, setCentralButton, targetCentral);
		...
		clearButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if (centralUser.getText().equals("NO_CENTRAL_USER")) {
					...
					numGroups.setText("Number of Connected Groups: " + network.numConnectedComponents());

					//Prompt
					Alert info = new Alert(Alert.AlertType.INFORMATION, "Network has been successfully cleared!");
					info.showAndWait();
		...
		addNewButton.setOnAction(event -> {
			...
			if (newUser.getText() == null || newUser.getText().equals("")) {
				Alert error = new Alert(Alert.AlertType.ERROR, "Please add username to the corresponding field");
				...
				try {
					...
					Alert info = new Alert(Alert.AlertType.INFORMATION,
							newUser.getText() + " has been successfully added to the network!");
					info.showAndWait();
					updateFriendList();
					
					//If the user already exist
				} catch (DuplicateKeyException e1) {
					Alert error = new Alert(Alert.AlertType.ERROR, "The username already exists in the network");
		...
		addButton.setOnAction(event -> {
			...
			if (newFriend1.getText() == null || newFriend2.getText() == null || newFriend1.getText().equals("")
					|| newFriend2.getText().equals("")) {
				Alert error = new Alert(Alert.AlertType.ERROR, "Please add username to both fields");
			...
			//If both friends to add friendship is the same user
				...
				Alert error = new Alert(Alert.AlertType.ERROR, "Please add 2 different usernames");
			...
			//Add the friendship between the two friends
				...
				try {
					reset.fire();
					network.addFriend(newFriend1.getText(), newFriend2.getText());

					//If there is no central user, set the first friend as central user
					if (centralUser.getText().equals("NO_CENTRAL_USER")) {
					...
					Alert info = new Alert(Alert.AlertType.INFORMATION, newFriend1.getText() + " and "
							+ newFriend2.getText() + " has been successfully added to the network!");
					info.showAndWait();
					
				//If the friendship between two friends already exist
				} catch (DuplicateEdgeException e1) {
					Alert error = new Alert(Alert.AlertType.ERROR, "The friendship already exists in the network");
		...
		removeButton.setOnAction(event -> {
			...
			} else if (removeFriend.getText() == null || removeFriend.getText().equals("")) {
				Alert error = new Alert(Alert.AlertType.ERROR, "Please add username to the corresponding field");
			...
			//If the friend intended to remove is the central user
				...
				Alert error = new Alert(Alert.AlertType.ERROR, "Removing central user is not allowed");
			...
			//Remove the friend
				...
				try {
					...
					Alert info = new Alert(Alert.AlertType.INFORMATION,
							removeFriend.getText() + " has been successfully removed from the network!");
					updateFriendList();
				...
				//If the user does not exist
					...
					Alert error = new Alert(Alert.AlertType.ERROR, "The username doesn't exist in the network");
		...
		removeFriendshipButton.setOnAction(event -> {
			...
			} else if (removeFriend1.getText() == null || removeFriend2.getText() == null
					|| removeFriend1.getText().equals("") || removeFriend2.getText().equals("")) {
				Alert error = new Alert(Alert.AlertType.ERROR, "Please add username to both fields");
			...
			} else if (removeFriend1.getText().equals(removeFriend2.getText())) {
				Alert error = new Alert(Alert.AlertType.ERROR,
			...
			//Remove friendship
				...
				try {
					...
					//Update network
					network.removeFriend(removeFriend1.getText(), removeFriend2.getText());
					updateFriendList();
					...
					Alert info = new Alert(Alert.AlertType.INFORMATION,
							removeFriend1.getText() + " and " + removeFriend2.getText() + " are no longer friend!");
					info.showAndWait();
					
				//If any of the users cannot be found
				} catch (VertexNotFoundException e1) {
					Alert error = new Alert(Alert.AlertType.ERROR, "One or both of the users couldn't be found");
				...
				//If friendship does not exist in the first place
					...
					Alert error = new Alert(Alert.AlertType.ERROR, "Friendship not found");
		...
		VBox removeFriendshipBox = new VBox(removeFriendshipButton, removeFriendshipBoxSub);
		...
		centerBox.getChildren().addAll(clearButton, addNewUserBox, addFriendsBox, removeFriendBox, removeFriendshipBox,
				historyListBox);
		...
		mainScene.addEventFilter(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
			public void handle(KeyEvent ke) {
				final KeyCombination keyComb = new KeyCodeCombination(KeyCode.U, KeyCombination.CONTROL_DOWN);
				if (keyComb.match(ke)) {
		...
		VBox bottomBox = new VBox(new Label("@ateam-69 CS400 Fall 2019 University of Wisconsin - Madison"));
